...
module TooltipDemo = {
  ...
  let make = (
    ...
    let tooltipLabel = if hideLabel {
      React.null
    } else {
      <div className="font-medium"> {label->React.string} </div>
    }
    ...
    if payload->Array.length == 0 {
      ...
      <div
        className={`border-border/50 bg-background grid min-w-[8rem] items-start gap-1.5 rounded-lg border px-2.5 py-1.5 text-xs shadow-xl transition-all ease-in-out hover:-translate-y-0.5 ${className}`}
      >
        {nestLabel ? React.null : tooltipLabel}
        <div className="grid gap-1.5">
          ...
          ->Array.mapWithIndex((item, index) => {
            ...
            let itemClass = switch indicator {
            | Dot => "items-center"
            | _ => "items-stretch"
            }
            <div
              key={index->Int.toString}
              className={`[&>svg]:text-muted-foreground flex w-full gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5 ${itemClass}`}
            >
              {hideIndicator
                ? React.null
                : <div
              ...
              >
                <div className="grid gap-1.5">
                  {nestLabel ? tooltipLabel : React.null}
...
let make = () =>
  <div
    className="text-foreground grid aspect-video w-full max-w-md justify-center md:grid-cols-2 [&>div]:relative [&>div]:flex [&>div]:h-[137px] [&>div]:w-[224px] [&>div]:items-center [&>div]:justify-center [&>div]:p-4"
  >
    <div>
