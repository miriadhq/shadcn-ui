...
let make = () =>
  <div className="flex flex-wrap gap-2">
    {sides
    ->Array.map(side => {
      let direction = if side === "bottom" {None} else {Some(side)}
      <Drawer key={side} ?direction>
        ...
        <Drawer.Content className="data-[vaul-drawer-direction=bottom]:max-h-[50vh] data-[vaul-drawer-direction=top]:max-h-[50vh]">
          <Drawer.Header>
