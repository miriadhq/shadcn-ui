...
let make = () =>
  <div className="flex w-full max-w-xl flex-col gap-6">
    <Item.Group className="grid grid-cols-3 gap-4">
      {models
      ->Array.map(model =>
        <Item key={model.name} variant=Item.Variant.Outline>
          <Item.Header>
            <img
              src={model.image}
              alt={model.name}
          ...
          <Item.Content>
            <Item.Title> {model.name->React.string} </Item.Title>
            <Item.Description> {model.description->React.string} </Item.Description>
          </Item.Content>
        </Item>
      )
      ->React.array}
