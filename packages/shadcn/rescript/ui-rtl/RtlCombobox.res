  ...
  let make = (
    ...
    >
      ...
      <div
        ...
        role="group"
        className="[&>kbd]:rounded-[calc(var(--radius)-5px)] [&>svg:not([class*='size-'])]:size-4 cursor-text flex font-medium gap-2 group-data-[disabled=true]/input-group:opacity-50 h-auto has-[>button]:me-[-0.3rem] has-[>kbd]:me-[-0.15rem] items-center justify-center order-last pe-2 py-1.5 select-none text-muted-foreground text-sm"
      >
        {showTrigger
          ? <BaseUi.Combobox.Trigger
...
module Item = {
  ...
  let make = (
    ...
    <BaseUi.Combobox.Item
      ...
      className={cn(
        "data-highlighted:bg-accent data-highlighted:text-accent-foreground not-data-[variant=destructive]:data-highlighted:**:text-accent-foreground relative flex w-full cursor-default items-center gap-2 rounded-md py-1 pe-8 ps-1.5 text-sm outline-hidden select-none data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className,
      )}
    >
      {children}
